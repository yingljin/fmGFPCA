...
comp_list_allM[[1]] %>%
  ...
  ggplot()+
  geom_point(aes(x=t, y=eta, col="True"), size = 0.1, na.rm = T)+
  geom_point(aes(x=t, y=eta_comp, col="Computed"), size = 0.1, 
             na.rm = T)+
  geom_point(aes(x=t, y=Yobs, col = "Observed"), size = 0.1,
             na.rm = T)+
  facet_grid(rows = vars(id), cols = vars(visit))+
  labs(title = "gmFPCA", colour="")
```
```{r}
comp_list_allM_ref[[1]] %>%
  filter(id %in% c(30, 32, 37, 38)) %>% 
  mutate_at(vars(starts_with("eta")), 
            function(x)(exp(x)/(1+exp(x)))) %>%
  mutate(eta=ifelse(is.na(Yobs), NA, eta),
  ...
  geom_point(aes(x=t, y=Yobs, col = "Observed"), size = 0.1,
             na.rm = T)+
  facet_grid(rows = vars(id), cols = vars(visit))+
  labs(title = "GLMMadaptive", colour="")
```
